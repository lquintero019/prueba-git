...
<?php
    ...
    if($_POST['tipo']==1){
		...
		$rutaEnServidor='../../../catalog/vegetales/images';
		$rutaTemporal=$_FILES['img']['tmp_name'];
		$nombreImagen=$_FILES['img']['name'];
		$rutaDestino=$rutaEnServidor.'/'.$nombreImagen;
		move_uploaded_file($rutaTemporal,$rutaDestino);

	  $sql1="UPDATE vegetales SET nombre='$nombre', img='$nombreImagen' WHERE id='$id'";
	  $rs1 = mysql_query($sql1,$conexion);
		if ($rs1) {
        	...
        	echo '<p><a href="vegetales.php"><button class="ui button">Aceptar</button></a></p>';
		}
		else{
			echo "error";
    ...
    if($_POST['tipo']==2){
    	$numrow= "SELECT COUNT(num_pasos) as fila  FROM ingre_proce_vegetales WHERE id_vegetales=$_POST[id]";
    	$numrows= mysql_query($numrow);
    	$row = mysql_fetch_assoc($numrows);
        for ($i=1; $i <=$row['fila'] ; $i++) { 
        	$ingre="ingre".$i;
	    	$sqlIngre='UPDATE ingre_proce_vegetales SET ingrediente ="'.$_POST[$ingre].'"  WHERE id_vegetales= "'.$_POST["id"].'" AND num_pasos="'.$i.'"';
    ...
    if($_POST['tipo']==3){
      $numrow= "SELECT COUNT(num_pasos) as fila  FROM proce_ingre_vegetales WHERE id_vegetal=$_POST[id]";
    	$numrows= mysql_query($numrow);
    	$row = mysql_fetch_assoc($numrows);
        for ($i=1; $i <=$row['fila'] ; $i++) { 
        	$proce="proce".$i;
	    	$sqlProce='UPDATE proce_ingre_vegetales SET proceso ="'.$_POST[$proce].'"  WHERE id_vegetal= "'.$_POST["id"].'" AND num_pasos="'.$i.'"';
