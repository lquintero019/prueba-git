...
<?php
    ...
    if($_POST['tipo']==1){
		...
		$rutaEnServidor='../../../catalog/bebidas/images';
		$rutaTemporal=$_FILES['img']['tmp_name'];
		$nombreImagen=$_FILES['img']['name'];
		$rutaDestino=$rutaEnServidor.'/'.$nombreImagen;
		move_uploaded_file($rutaTemporal,$rutaDestino);

	  $sql1="UPDATE bebidas SET nombre='$nombre', img='$nombreImagen' WHERE id='$id'";
	  $rs1 = mysql_query($sql1,$conexion);
		if ($rs1) {
        	...
        	echo '<p><a href="bebidas.php"><button class="ui button">Aceptar</button></a></p>';
		}
		else{
			echo "error";
    ...
    if($_POST['tipo']==2){
    	$numrow= "SELECT COUNT(num_pasos) as fila  FROM ingre_proce_bebidas WHERE id_bebida=$_POST[id]";
    	$numrows= mysql_query($numrow);
    	$row = mysql_fetch_assoc($numrows);
        for ($i=1; $i <=$row['fila'] ; $i++) { 
        	$ingre="ingre".$i;
	    	$sqlIngre='UPDATE ingre_proce_bebidas SET ingrediente ="'.$_POST[$ingre].'"  WHERE id_bebida= "'.$_POST["id"].'" AND num_pasos="'.$i.'"';
    ...
    if($_POST['tipo']==3){
      $numrow= "SELECT COUNT(num_pasos) as fila  FROM proce_ingre_bebidas WHERE id_bebida=$_POST[id]";
    	$numrows= mysql_query($numrow);
    	$row = mysql_fetch_assoc($numrows);
        for ($i=1; $i <=$row['fila'] ; $i++) { 
        	$proce="proce".$i;
	    	$sqlProce='UPDATE proce_ingre_bebidas SET proceso ="'.$_POST[$proce].'" WHERE id_bebida= "'.$_POST["id"].'" AND num_pasos="'.$i.'"';
