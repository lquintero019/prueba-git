            ...
            <?php foreach ($pastas as $p):?>
                ...
                <div id="p<?= $pagina;?>" class="contenedor <?= ($p->id==1)?'_current':'';?>" style="<?=($p->id==1)?'':'display: none;';?>">
                    ...
                    <div id="recipe-ingredients">
                        ...
                        <?php 
                            $ingredientes=array();
                            $sql = "SELECT * FROM ingre_proce_pastas WHERE id_pastas = $p->id ORDER BY num_pasos ASC";
                            $resultado= $con->query($sql,MYSQLI_USE_RESULT);
                            while($campos=$resultado->fetch_object()){ 
                                    $ingredientes[]=$campos;
                            }
                    ...
                    <div id="recipe-directions"> 
                        ...
                        <?php 
                            $proceso=array();
                            $sql = "SELECT * FROM proce_ingre_pastas WHERE id_pasta = $p->id ORDER BY num_pasos ASC";
                            $resultado= $con->query($sql,MYSQLI_USE_RESULT);
                            while($campos=$resultado->fetch_object()){ 
                                    $proceso[]=$campos;
                            }
