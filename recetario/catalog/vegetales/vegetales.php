    ...
    <form id="form1">
        <div class="content">
            <div id="cuadro" class="cuerpo">
                <h1><img src="images/logo-vegetal.png"></h1>
            <?php foreach ($vegetales as $c):?>
                 <? $pagina=$pagina+1; ?>
                <div id="p<?= $pagina;?>" class="contenedor <?= ($c->id==1)?'_current':'';?>" style="<?=($c->id==1)?'':'display: none;';?>">
                    <p><?= $c->nombre?></p>
                    <div id="img">
                    ...
                    <div id="recipe-ingredients">
                        ...
                        <?php 
                            $ingredientes=array();
                            $sql = "SELECT * FROM ingre_proce_vegetales WHERE id_vegetales = $c->id ORDER BY num_pasos ASC";
                            $resultado= $con->query($sql,MYSQLI_USE_RESULT);
                            while($campos=$resultado->fetch_object()){ 
                                    $ingredientes[]=$campos;
                            }
                    ...
                    <div id="recipe-directions"> 
                        ...
                        <?php 
                            $proceso=array();
                            $sql = "SELECT * FROM proce_ingre_vegetales WHERE id_vegetal = $c->id ORDER BY num_pasos ASC";
                            $resultado= $con->query($sql,MYSQLI_USE_RESULT);
                            while($campos=$resultado->fetch_object()){ 
                                    $proceso[]=$campos;
                            }
        ...
        $(function () {            
            $("#paginas").paginate({
                ...
                onChange: function (page) {
                    $('._current', '#cuadro').removeClass('_current').hide();
