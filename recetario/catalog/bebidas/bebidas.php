    ...
    <form id="form1">
        <div class="content">
            <div id="cuadro" class="cuerpo">
                <h1><img src="images/logo-bebidas.png"></h1>
            <?php foreach ($bebida as $b):?>
                <? $pagina=$pagina+1; ?>
                <div id="p<?= $pagina;?>" class="contenedor <?= ($b->id==1)?'_current':'';?>" style="<?=($b->id==1)?'':'display: none;';?>">
                    <p><?= $b->nombre?></p>
                    <div id="img">
                    ...
                    <div id="recipe-ingredients">
                        ...
                        <?php 
                            $ingredientes=array();
                            $sql = "SELECT * FROM ingre_proce_bebidas WHERE id_bebida = $b->id ORDER BY num_pasos ASC";
                            $resultado= $con->query($sql,MYSQLI_USE_RESULT);
                            while($campos=$resultado->fetch_object()){ 
                                    $ingredientes[]=$campos;
                            }
                    ...
                    <div id="recipe-directions"> 
                        ...
                        <?php 
                            $proceso=array();
                            $sql = "SELECT * FROM proce_ingre_bebidas WHERE id_bebida = $b->id ORDER BY num_pasos ASC";
                            $resultado= $con->query($sql,MYSQLI_USE_RESULT);
                            while($campos=$resultado->fetch_object()){ 
                                    $proceso[]=$campos;
                            }
        ...
        $(function () {            
            $("#paginas").paginate({
                ...
                onChange: function (page) {
                    ...
                    $('#p' + page).addClass('_current').show().fadeIn(1000);
                }
            });
        });
	</script>