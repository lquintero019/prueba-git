    ...
    <form id="form1">
        <div class="content">
            <div id="cuadro" class="cuerpo">
                <h1><img src="images/logo-carnes.png"></h1>
            <?php foreach ($carnes as $c):?>
                ...
                <div id="p<?= $pagina;?>" class="contenedor <?= ($c->id==1)?'_current':'';?>" style="<?=($c->id==1)?'':'display: none;';?>">
                    ...
                    <div id="recipe-ingredients">
                        ...
                        <?php 
                            $ingredientes=array();
                            $sql = "SELECT * FROM ingre_proce_carnes WHERE id_carne = $c->id ORDER BY num_pasos ASC";
                            $resultado= $con->query($sql,MYSQLI_USE_RESULT);
                            while($campos=$resultado->fetch_object()){ 
                                    $ingredientes[]=$campos;
                            }
                    ...
                    <div id="recipe-directions"> 
                        ...
                        <?php 
                            $proceso=array();
                            $sql = "SELECT * FROM proce_ingre_carnes WHERE id_carne = $c->id ORDER BY num_pasos ASC";
                            $resultado= $con->query($sql,MYSQLI_USE_RESULT);
                            while($campos=$resultado->fetch_object()){ 
                                    $proceso[]=$campos;
                            }
